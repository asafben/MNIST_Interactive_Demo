...
CHANGE_ARCHITECHTURE = False
IS_WEIGHTED = False
OPTIMIZER = 'grad'
QUESTION_NUMBER = 1

# For the switch-case function.
def set_values(bat, iter_num, rate, actv_func, architecture, weighted, optim):
    ...
    ACTIVATION_FUNC = actv_func

    global CHANGE_ARCHITECHTURE
    CHANGE_ARCHITECHTURE = architecture
...
def build_network(images):
    ...
    with tf.name_scope('LeNet'):
        ...
        elif IS_WEIGHTED == True:
            net = slim.conv2d(net, 16, [5, 5],weights_initializer=tf.truncated_normal_initializer(stddev=12), scope='conv1')
            net = slim.max_pool2d(net, [2, 2], 2, scope='pool1')
            net = slim.conv2d(net, 32, [5, 5],weights_initializer=tf.truncated_normal_initializer(stddev=0.15), scope='conv2')
            net = slim.max_pool2d(net, [2, 2], 2, scope='pool2')
        ...
        predictions = slim.fully_connected(net, 10, activation_fn=ACTIVATION_FUNC,
                                           scope='fc4')
...
def train_net(basedir):
    ...
    # Pick Optimizer:
    if OPTIMIZER == 'grad':
        train_step = tf.train.GradientDescentOptimizer(learning_rate=LEARNING_RATE).minimize(compute_loss)

    elif OPTIMIZER == 'adam':
        train_step = tf.train.AdamOptimizer(learning_rate=LEARNING_RATE).minimize(compute_loss)

    elif OPTIMIZER == 'adag':
        train_step = tf.train.AdagradOptimizer(learning_rate=LEARNING_RATE).minimize(compute_loss)
...
if __name__ == "__main__":
    ...
    ## set_values signature: (batch, iter_num, learning_rate, actv_func, architecture, weighted, optimizer) ##
    if QUESTION_NUMBER == 1:
        set_values(32, 500, 0.1, None, False, False, 'grad')
        # set_values(1, 2000, 0.1, None, False, False, 'grad')
        # set_values(64, 500, 0.1, None, False, False, 'grad')
        # set_values(256, 500, 0.1, None, False, False, 'grad')

    elif QUESTION_NUMBER == 2:
        set_values(32, 500, 0.1, None, False, False, 'grad')
        # set_values(32, 500, 0.01, None, False, False, 'grad')
        # set_values(32, 500, 0.5, None, False, False, 'grad')
        # set_values(32, 500, 1, None, False, False, 'grad')

    elif QUESTION_NUMBER == 3:
        set_values(32, 500, 0.1, None, False, False, 'grad')
        # set_values(32, 500, 0.1, None, False, False, 'adam')
        # set_values(32, 500, 0.1, None, False, False, 'adag')

    elif QUESTION_NUMBER == 4:
        set_values(32, 500, 0.1, None, False, False, 'grad')
        # set_values(32, 500, 0.1, tf.nn.relu, False, False, 'grad')
        # set_values(32, 500, 0.1, tf.sigmoid, False, False, 'grad')

    elif QUESTION_NUMBER == 5:
        set_values(32, 500, 0.1, None, False, True, 'grad')

    elif QUESTION_NUMBER == 6:
        set_values(32, 500, 0.1, None, True, False, 'grad')

    print(BATCH_SIZE, ITERATION_NUM, LEARNING_RATE, ACTIVATION_FUNC, CHANGE_ARCHITECHTURE, IS_WEIGHTED, OPTIMIZER, QUESTION_NUMBER)
